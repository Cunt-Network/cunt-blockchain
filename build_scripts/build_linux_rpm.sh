...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="cunt-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="cunt-blockchain-linux-arm64"
fi
...
electron-packager . cunt-blockchain --asar.unpack="**/daemon/**" --platform=linux \
--icon=src/assets/img/Cunt.icns --overwrite --app-bundle-id=net.cunt.blockchain \
--appVersion=$CUNT_INSTALLER_VERSION --executable-name=cunt-blockchain
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create cunt-blockchain-$CUNT_INSTALLER_VERSION.rpm"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Cunt-Network/cunt-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $CUNT_INSTALLER_VERSION \
  --license ../LICENSE --options.bin cunt-blockchain --options.name cunt-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
