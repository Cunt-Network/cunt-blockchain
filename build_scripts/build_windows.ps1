...
Write-Output "   ---"
# download.cuntnetwork.org is the CDN url behind all the files that are actually on pypi.cuntnetwork.org/simple now
...
Write-Output "   ---"
Copy-Item "dist\daemon" -Destination "..\cunt-blockchain-gui\packages\gui\" -Recurse
...
jq --arg VER "$env:CUNT_INSTALLER_VERSION" '.version=$VER' package.json > temp.json
rm package.json
...
Write-Output "   ---"
Copy-Item ".\Cunt-win32-x64" -Destination "$env:GITHUB_WORKSPACE\cunt-blockchain-gui\" -Recurse
Copy-Item ".\release-builds" -Destination "$env:GITHUB_WORKSPACE\cunt-blockchain-gui\" -Recurse
