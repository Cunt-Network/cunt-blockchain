...
from cunt.consensus.block_rewards import calculate_base_farmer_reward
from cunt.pools.pool_wallet import PoolWallet
from cunt.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
...
from cunt.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from cunt.util.byte_types import hexstr_to_bytes
from cunt.util.ints import uint32, uint64, uint8
from cunt.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
from cunt.util.path import path_from_root
...
from cunt.wallet.derive_keys import master_sk_to_singleton_owner_sk, master_sk_to_wallet_sk_unhardened, MAX_POOL_WALLETS
from cunt.wallet.rl_wallet.rl_wallet import RLWallet
from cunt.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from cunt.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
class WalletRpcApi:
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("vag_target_address")
        pool_target = config["pool"].get("vag_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from cunt.pools.pool_wallet_info import initial_pool_state_from_dict
