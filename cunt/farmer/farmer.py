...
from cunt.pools.pool_config import PoolWalletConfig, load_pool_config, add_auth_key
...
from cunt.server.server import ssl_context_for_root
...
from cunt.util.bech32m import decode_puzzle_hash
from cunt.util.byte_types import hexstr_to_bytes
from cunt.util.config import load_config, save_config, config_path_for_filename, get_config_lock
...
from cunt.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'cunt keys generate' or open the UI."
        ...
        if "vag_target_address" not in self.config:
            self.config = config["farmer"]
        if "vag_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "vag_target_address" not in self.config or "vag_target_address" not in self.pool_config:
            log.debug("vag_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["vag_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["vag_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]):
        with get_config_lock(self._root_path, "config.yaml"):
            ...
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["vag_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["vag_target_address"] = pool_target_encoded
    ...
    async def get_cached_harvesters(self, connection: WSCuntConnection) -> HarvesterCacheEntry:
