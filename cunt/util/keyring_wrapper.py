...
from cunt.util.default_root import DEFAULT_KEYS_ROOT_PATH
from cunt.util.file_keyring import FileKeyring
from cunt.util.misc import prompt_yes_no
...
# the new passphrase.
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ cunt passphrase set # all the cool kids are doing it!"

MASTER_PASSPHRASE_SERVICE_NAME = "Cunt Passphrase"
MASTER_PASSPHRASE_USER_NAME = "Cunt Passphrase"
...
def check_legacy_keyring_keys_present(keyring: LegacyKeyring) -> bool:
    ...
    from cunt.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def __init__(self, keys_root_path: Path = DEFAULT_KEYS_ROOT_PATH, force_legacy: bool = False):
        ...
        """
        from cunt.util.keychain import KeyringNotSet
    ...
    def _get_initial_cached_passphrase(self) -> str:
        ...
        """
        from cunt.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'cunt passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase? (y/n) ")
            if response:
                from cunt.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'cunt passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from cunt.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    def migrate_legacy_keyring_interactive(self):
        ...
        """
        from cunt.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
