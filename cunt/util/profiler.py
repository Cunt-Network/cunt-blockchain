...
# the output will be printed to your cunt root path, e.g. ~/.cunt/mainnet/profile/
# to analyze the profile, run:

#   python cunt/utils/profiler.py ~/.cunt/mainnet/profile | less -r

# this will print CPU usage of the cunt full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python cunt/utils/profiler.py ~/.cunt/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int):
        ...
        output_file = "cunt-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
