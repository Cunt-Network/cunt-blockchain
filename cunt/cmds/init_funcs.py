...
from cunt.ssl.create_ssl import generate_ca_signed_cert, get_cunt_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    if "vag_target_address" not in config["farmer"]:
        print(f"Setting the vag destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "vag_target_address" not in config["pool"]:
        print(f"Setting the vag destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    cunt_ca_crt_path.write_bytes(cunt_ca_crt)
    cunt_ca_key_path.write_bytes(cunt_ca_key)
