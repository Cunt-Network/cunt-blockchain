...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if cunt_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return cunt_init(root_path, fix_ssl_permissions=fix_ssl_permissions)
...
def cunt_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    """
