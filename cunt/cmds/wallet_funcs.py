...
from cunt.cmds.show import print_connections
from cunt.cmds.units import units
from cunt.rpc.wallet_rpc_client import WalletRpcClient
from cunt.server.outbound_message import NodeType
from cunt.server.start_wallet import SERVICE_NAME
...
def print_transaction(tx: TransactionRecord, verbose: bool, name, address_prefix: str, mojo_per_unit: int) -> None:
    if verbose:
        ...
        cunt_amount = Decimal(int(tx.amount)) / mojo_per_unit
        to_address = encode_puzzle_hash(tx.to_puzzle_hash, address_prefix)
        print(f"Transaction {tx.name}")
        print(f"Status: {'Confirmed' if tx.confirmed else ('In mempool' if tx.is_in_mempool() else 'Pending')}")
        print(f"Amount {'sent' if tx.sent else 'received'}: {cunt_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(fee * units["cunt"]))
    final_amount: uint64
    if typ == WalletType.STANDARD_WALLET:
        final_amount = uint64(int(amount * units["cunt"]))
    ...
    while time.time() - start < 10:
        ...
        if len(tx.sent_to) > 0:
            ...
            print(f"Do cunt wallet get_transaction -f {fingerprint} -tx 0x{tx_id} to get status")
            return None
...
async def make_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee: int = int(Decimal(args["fee"]) * units["cunt"])
        ...
        for item in [*offers, *requests]:
            ...
            if confirmation not in ["y", "yes"]:
                ...
                if offer is not None:
                    ...
                    print(f"Use cunt wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view status")
                else:
...
async def print_trade_record(record, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Fees: {Decimal(offer.bundle.fees()) / units['cunt']}")
    print("---------------")
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if not examine_only:
        ...
        if confirmation in ["y", "yes"]:
            ...
            print(f"Use cunt wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if confirmation in ["y", "yes"]:
        ...
        if secure:
            print(f"Use cunt wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG, WalletType.RATE_LIMITED]:
        return address_prefix, units["cunt"]
    return "", units["mojo"]
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'cunt keys generate' or import a key")
        return None
