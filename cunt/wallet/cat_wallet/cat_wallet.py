...
from cunt.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from cunt.wallet.cat_wallet.lineage_store import CATLineageStore
from cunt.wallet.lineage_proof import LineageProof
from cunt.wallet.payment import Payment
from cunt.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            cunt_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        cunt_tx = dataclasses.replace(cunt_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(cunt_tx)
    ...
    async def create_tandem_vag_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            cunt_coins = await self.standard_wallet.select_coins(fee)
            origin_id = list(cunt_coins)[0].name()
            cunt_tx = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            assert cunt_tx.spend_bundle is not None

            message = None
            for spend in cunt_tx.spend_bundle.coin_spends:
            ...
            cunt_coins = await self.standard_wallet.select_coins(fee)
            selected_amount = sum([c.amount for c in cunt_coins])
            cunt_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some VAG as part of this
        regular_cunt_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_cunt_to_claim = payment_amount

        need_cunt_transaction = (fee > 0 or regular_cunt_to_claim > 0) and (fee - regular_cunt_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_cunt_transaction:
                    if fee > regular_cunt_to_claim:
                        cunt_tx, _ = await self.create_tandem_vag_tx(
                            fee, uint64(regular_cunt_to_claim), announcement_to_assert=announcement
                        )
                    ...
                    elif regular_cunt_to_claim > fee:
                        cunt_tx, _ = await self.create_tandem_vag_tx(fee, uint64(regular_cunt_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        cunt_spend_bundle = SpendBundle([], G2Element())
        if cunt_tx is not None and cunt_tx.spend_bundle is not None:
            cunt_spend_bundle = cunt_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, cunt_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
        ...
        if cunt_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=cunt_tx.confirmed_at_height,
                    created_at_time=cunt_tx.created_at_time,
                    to_puzzle_hash=cunt_tx.to_puzzle_hash,
                    amount=cunt_tx.amount,
                    fee_amount=cunt_tx.fee_amount,
                    confirmed=cunt_tx.confirmed,
                    sent=cunt_tx.sent,
                    spend_bundle=None,
                    additions=cunt_tx.additions,
                    removals=cunt_tx.removals,
                    wallet_id=cunt_tx.wallet_id,
                    sent_to=cunt_tx.sent_to,
                    trade_id=cunt_tx.trade_id,
                    type=cunt_tx.type,
                    name=cunt_tx.name,
                    memos=[],
                )
