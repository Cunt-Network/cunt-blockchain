...
from cunt.wallet.puzzles.puzzle_utils import (
...
from cunt.wallet.secret_key_store import SecretKeyStore
from cunt.wallet.sign_coin_spends import sign_coin_spends
...
from cunt.wallet.util.wallet_types import WalletType, AmountWithPuzzlehash
...
class Wallet:
    ...
    async def create_spend_bundle_relative_cunt(self, cunt_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if cunt_amount < 0:
            utxos = await self.select_coins(abs(cunt_amount), exclude)
        else:
        ...
        cunt_amount = spend_value + cunt_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(cunt_amount), "memos": []}
                ]
