...
from cunt.util.bech32m import bech32_encode, bech32_decode, convertbits
from cunt.util.ints import uint64
from cunt.wallet.util.puzzle_compression import (
...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting VAG
        coins: List[Coin],
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "vag" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
        ...
        # Then we add a potential fee as pending VAG
        fee: int = sum(c.amount for c in all_removals) - sum(c.amount for c in all_additions)
        if fee > 0:
            pending_dict.setdefault("vag", 0)
            pending_dict["vag"] += fee
