...
from cunt.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
...
from cunt.util.ints import uint32, uint128
from cunt.util.keychain import Keychain
from cunt.util.lru_cache import LRUCache
from cunt.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from cunt.util.path import mkdir, path_from_root
from cunt.wallet.block_record import HeaderBlockRecord
from cunt.wallet.derivation_record import DerivationRecord
from cunt.wallet.settings.settings_objects import BackupInitialized
from cunt.wallet.transaction_record import TransactionRecord
from cunt.wallet.util.backup_utils import open_backup_file
from cunt.wallet.util.wallet_types import WalletType
from cunt.wallet.wallet_action import WalletAction
from cunt.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSCuntConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSCuntConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSCuntConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSCuntConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSCuntConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSCuntConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
