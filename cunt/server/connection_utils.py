...
async def send_all_first_reply(
    func: str, arg: Any, peers: List[WSCuntConnection], timeout=15
) -> Optional[Tuple[Any, WSCuntConnection]]:
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""

    async def do_func(peer_x: WSCuntConnection, func_x: str, arg_x: Any):
...
async def send_to_random(func: str, arg: Any, peers: List[WSCuntConnection]) -> Optional[Tuple[Any, WSCuntConnection]]:
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""
