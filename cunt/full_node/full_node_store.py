...
from cunt.consensus.pot_iterations import calculate_sp_interval_iters
...
from cunt.types.generator_types import CompressorArg
...
class FullNodeStore:
    ...
    def add_to_future_sp(self, signage_point: SignagePoint, index: uint8):
        ...
        log.info(f"Don't have rc hash {signage_point.rc_vdf.challenge}. vaghing signage point {index}.")
    ...
    def new_finished_sub_slot(
        ...
        if peak is not None and peak.total_iters > last_slot_iters:
            ...
            if peak.reward_infusion_new_challenge != rc_challenge:
                ...
                log.info(f"Don't have challenge hash {rc_challenge}, vaghing EOS")
                return None
