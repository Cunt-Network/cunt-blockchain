...
import cunt.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from cunt.consensus.block_creation import unfinished_block_to_full_block
from cunt.consensus.block_record import BlockRecord
from cunt.consensus.blockchain import Blockchain, ReceiveBlockResult
...
from cunt.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from cunt.consensus.multiprocess_validation import PreValidationResult
from cunt.consensus.pot_iterations import calculate_sp_iters
from cunt.full_node.block_store import BlockStore
from cunt.full_node.lock_queue import LockQueue, LockClient
...
from cunt.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
...
from cunt.full_node.sync_store import SyncStore
from cunt.full_node.weight_proof import WeightProofHandler
from cunt.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from cunt.protocols.full_node_protocol import (
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
    RespondSignagePoint,
...
from cunt.protocols.wallet_protocol import CoinState, CoinStateUpdate
from cunt.server.node_discovery import FullNodePeers
from cunt.server.outbound_message import Message, NodeType, make_msg
from cunt.server.peer_store_resolver import PeerStoreResolver
...
from cunt.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from cunt.types.transaction_queue_entry import TransactionQueueEntry
from cunt.types.unfinished_block import UnfinishedBlock
from cunt.util import cached_bls
from cunt.util.bech32m import encode_puzzle_hash
from cunt.util.check_fork_next_block import check_fork_next_block
from cunt.util.condition_tools import pkm_pairs
from cunt.util.config import PEER_DB_PATH_KEY_DEPRECATED, process_config_start_method
from cunt.util.db_wrapper import DBWrapper
from cunt.util.errors import ConsensusError, Err, ValidationError
from cunt.util.ints import uint8, uint32, uint64, uint128
from cunt.util.path import mkdir, path_from_root
from cunt.util.safe_cancel_task import cancel_task_safe
from cunt.util.profiler import profile_task
from datetime import datetime
from cunt.util.db_synchronous import db_synchronous_on
from cunt.util.db_version import lookup_db_version
...
class FullNode:
    ...
    async def _start(self):
        ...
        # Never use pragma synchronous=OFF in Cunt.
    ...
    def set_server(self, server: CuntServer):
        ...
        elif self.config["port"] == 18690:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.cuntcoin.com")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSCuntConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSCuntConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSCuntConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSCuntConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSCuntConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSCuntConnection):
    ...
    def remove_subscriptions(self, peer: ws.WSCuntConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue, peers_with_peak: List[ws.WSCuntConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSCuntConnection, List[FullBlock]]] = asyncio.Queue(
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: ws.WSCuntConnection = self.server.all_connections[peer]
    ...
    async def peak_post_processing(
        ...
        record: BlockRecord,
        fork_height: uint32,
        peer: Optional[ws.WSCuntConnection],
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSCuntConnection] = None,
    ...
    async def respond_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'vag')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSCuntConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSCuntConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSCuntConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSCuntConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSCuntConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: ws.WSCuntConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
