...
from cunt.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from cunt.plotters.chiapos import get_chiapos_install_info, plot_cunt
from cunt.plotters.madmax import get_madmax_install_info, plot_madmax
from cunt.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in CUNT_ROOT.
    cunt_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within CUNT_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, cunt_plotter, "chiapos", "Cuntpos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_cunt(args, cunt_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, cunt_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, cunt_root_path, root_path)
