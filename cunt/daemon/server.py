...
from cunt.cmds.init_funcs import check_keys, cunt_init
from cunt.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from cunt.daemon.keychain_server import KeychainServer, keychain_commands
from cunt.daemon.windows_signal import kill
from cunt.plotters.plotters import get_available_plotters
from cunt.plotting.util import add_plot_directory
from cunt.server.server import ssl_context_for_root, ssl_context_for_server
from cunt.ssl.create_ssl import get_mozilla_ca_crt
from cunt.util.cunt_logging import initialize_logging
from cunt.util.config import load_config
from cunt.util.json_util import dict_to_json_str
from cunt.util.keychain import (
    Keychain,
...
from cunt.util.service_groups import validate_service
from cunt.util.setproctitle import setproctitle
from cunt.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Cunt.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "cunt": "cunt",
        "cunt_wallet": "start_wallet",
        "cunt_full_node": "start_full_node",
        "cunt_harvester": "start_harvester",
        "cunt_farmer": "start_farmer",
        "cunt_introducer": "start_introducer",
        "cunt_timelord": "start_timelord",
        "cunt_timelord_launcher": "timelord_launcher",
        "cunt_full_node_simulator": "start_simulator",
        "cunt_seeder": "start_seeder",
        "cunt_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Cunt will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
        else:
        ...
        try:
            if Keychain.master_passphrase_is_valid(key, force_reload=True):
                ...
                try:
                    ...
                    self.log.exception("Failed to migrate keys silently. Run `cunt keys migrate` manually.")
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["cunt", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("cunt daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered CUNT_ROOT
    os.environ["CUNT_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with CUNT_ROOT: {os.environ['CUNT_ROOT']}")
    ...
    if service_command == "cunt_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in cunt_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    cunt_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("cunt_daemon")
