...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the cunt python virtual environment."
  echo "Execute '. ./activate' before running."
  exit 1
fi

if [ "$(id -u)" = 0 ]; then
  echo "The Cunt Blockchain GUI can not be installed or run by the root user."
  exit 1
fi

# Allows overriding the branch or commit to build in cunt-blockchain-gui
SUBMODULE_BRANCH=$1
...
# https://github.com/Cunt-Network/cunt-blockchain/pull/10460#issuecomment-1054492495
...
if [ ! "$CI" ]; then
  ...
  # Set modified output of `cunt version` to version property of GUI's package.json
...
echo ""
echo "Cunt blockchain install-gui.sh completed."
echo ""
