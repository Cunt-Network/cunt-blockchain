...
import { /* byte_to_cunt_string, */ cunt_to_byte } from '../../../util/cunt';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {byte_to_cunt_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    const amountValue = Number.parseFloat(cunt_to_byte(amount));
    const feeValue = Number.parseFloat(cunt_to_byte(fee));
