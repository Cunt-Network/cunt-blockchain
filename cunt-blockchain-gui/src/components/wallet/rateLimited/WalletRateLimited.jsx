...
import { AlertDialog, Card, Flex } from '@cunt/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hevageck = /[\da-f]+$/gi;

    if (!hevageck.test(ip_val) || ip_val.value === '') {
    ...
    const cuntper_input = ip_parsed.limit;
    ...
    const cuntper_value = Number.parseInt(Number(cuntper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (cunt per interval):{' '}
                  {mojo_to_cunt_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = cunt_to_mojo(fee_input.value);
