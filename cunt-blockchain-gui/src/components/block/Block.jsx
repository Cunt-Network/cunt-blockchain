...
export default function Block() {
  ...
  const poolReward = mojo_to_cunt(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_cunt(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const cuntFees = blockRecord.fees
    ? mojo_to_cunt(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.cuntexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.cuntexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
