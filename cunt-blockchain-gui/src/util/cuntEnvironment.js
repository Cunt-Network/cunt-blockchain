...
const getCuntVersion = () => {
  let version = null;
  const exePath = getExecutablePath('cunt');
  // first see if we can get a cunt exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or cunt is on the path
    try {
      version = child_process
