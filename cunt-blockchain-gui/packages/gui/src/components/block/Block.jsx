...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@cunt/api-react'
...
export default function Block() {
  ...
  const poolReward = mojoToCunt(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToCunt(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const cuntFees = blockRecord.fees !== undefined
    ? mojoToCunt(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.cuntexplorer.org/blockchain/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.cuntexplorer.org/blockchain/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: cuntFees ? `${cuntFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <>
      ...
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Cunt blockchain
            </Trans>
          </Back>
        }
        action={
