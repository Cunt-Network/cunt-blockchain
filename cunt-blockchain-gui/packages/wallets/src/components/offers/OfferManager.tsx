...
import { Trade as TradeIcon } from '@cunt/icons';
import { useCancelOfferMutation, useGetOfferDataMutation, useGetWalletsQuery } from '@cunt/api-react';
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: vagFee } = methods.getValues();
    var fee = 0;

    if (cancelWithTransaction) {
      fee = Number.parseFloat(cuntToMojo(vagFee));
    }
      ...
      <Grid container>
        <Grid xs={12} md={6} lg={5} item>
          <CardHero>
            ...
            <Typography variant="body1">
              <Trans>
                Create an offer to exchange VAG or other tokens. View an offer to inspect and accept an offer made by another party.
              </Trans>
            </Typography>
