...
import { type OfferSummaryRecord  } from '@cunt/api';
import {
  Flex,
  StateColor,
  mojoToCunt,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'vag' ? Number(mojoToCunt(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'vag' ? Number(mojoToCunt(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
