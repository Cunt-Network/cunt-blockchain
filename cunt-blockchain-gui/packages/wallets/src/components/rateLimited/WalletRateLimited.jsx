...
import { AlertDialog, Card, Flex, cuntToMojo, mojoToCuntLocaleString } from '@cunt/core';
import {
  send_transaction,
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToCuntLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = cuntToMojo(amount_input.value);
    const fee = cuntToMojo(fee_input.value);
