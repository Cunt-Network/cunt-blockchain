...
import { AlertDialog, cuntToMojo } from '@cunt/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      cuntper_input.value === '' ||
      Number(cuntper_input.value) === 0 ||
      !Number(cuntper_input.value) ||
      isNaN(Number(cuntper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const cuntper = cuntToMojo(cuntper_input.value);
    ...
    // var fee = cuntToMojo(fee_input.value);
