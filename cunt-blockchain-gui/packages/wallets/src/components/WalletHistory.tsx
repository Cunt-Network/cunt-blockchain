...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToCuntLocaleString, mojoToCATLocaleString } from '@cunt/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@cunt/api-react';
import styled from 'styled-components';
import type { Row } from '@cunt/core';
import { WalletType, TransactionType } from '@cunt/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToCuntLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToCuntLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
