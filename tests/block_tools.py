...
from cunt.cmds.init_funcs import create_all_ssl, create_default_cunt_config
from cunt.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from cunt.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from cunt.util.errors import Err
from cunt.full_node.generator import setup_generator_args
from cunt.full_node.mempool_check_conditions import GENERATOR_MOD
from cunt.plotting.create_plots import create_plots, PlotKeys
...
from cunt.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from cunt.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
from cunt.plotting.manager import PlotManager
from cunt.server.server import ssl_context_for_server
...
from cunt.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from cunt.types.blockchain_format.pool_target import PoolTarget
from cunt.types.blockchain_format.program import INFINITE_COST
from cunt.types.blockchain_format.proof_of_space import ProofOfSpace
from cunt.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
...
from cunt.types.generator_types import BlockGenerator, CompressorArg
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="cunt-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `cunt generate keys`")
        ...
        try:
            if plot_keys is None:
                ...
                if pool_contract_puzzle_hash is None:
                    ...
                    pool_address = encode_puzzle_hash(pool_contract_puzzle_hash, "vag")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("CUNT_ROOT", "~/.cunt/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
