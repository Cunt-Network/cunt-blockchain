...
from cunt.clvm.spend_sim import SpendSim, SimClient
...
from cunt.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    construct_cat_puzzle,
    SpendableCAT,
    unsigned_spend_bundle_for_spendable_cats,
)
from cunt.wallet.payment import Payment
from cunt.wallet.trading.offer import Offer, NotarizedPayment
...
class TestOfferLifecycle:
    ...
    # This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/VAG coins
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            cunt_coins: List[Coin] = all_coins[None]
            red_coins: List[Coin] = all_coins["red"]
            blue_coins: List[Coin] = all_coins["blue"]

            # Create an VAG Offer for RED
            cunt_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            cunt_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                cunt_requested_payments, cunt_coins
            )
            cunt_announcements: List[Announcement] = Offer.calculate_announcements(cunt_requested_payments)
            cunt_secured_bundle: SpendBundle = self.generate_secure_bundle(cunt_coins, cunt_announcements, 1000)
            cunt_offer = Offer(cunt_requested_payments, cunt_secured_bundle)
            assert not cunt_offer.is_valid()

            # Create a RED Offer for VAG
            ...
            new_offer = Offer.aggregate([cunt_offer, red_offer])
            ...
            # Create yet another offer of BLUE for VAG and RED
            ...
            assert new_offer.summary() == (
                ...
                {"vag": 900, str_to_tail_hash("red").hex(): 350},
            )
