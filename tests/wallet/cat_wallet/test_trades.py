...
from cunt.util.ints import uint16, uint64
from cunt.wallet.cat_wallet.cat_wallet import CATWallet
from cunt.wallet.trading.offer import Offer
from cunt.wallet.trading.trade_status import TradeStatus
...
class TestCATTrades:
    ...
    async def test_cat_trades(self, wallets_prefarm):
        ...
        MAKER_CUNT_BALANCE = 20 * 1000000000000 - 100
        TAKER_CUNT_BALANCE = 20 * 1000000000000 - 100
        await time_out_assert(25, wallet_maker.get_confirmed_balance, MAKER_CUNT_BALANCE)
        await time_out_assert(25, wallet_taker.get_unconfirmed_balance, TAKER_CUNT_BALANCE)
        ...
        # cunt_for_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cunt_for_cat, fee=uint64(1))
        await asyncio.sleep(1)
        ...
        MAKER_CUNT_BALANCE -= 2  # -1 and -1 for fee
        MAKER_NEW_CAT_BALANCE += 2
        TAKER_CUNT_BALANCE += 0  # +1 and -1 for fee
        TAKER_NEW_CAT_BALANCE -= 2

        await time_out_assert(15, wallet_taker.get_unconfirmed_balance, TAKER_CUNT_BALANCE)
        ...
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_CUNT_BALANCE)
        await time_out_assert(15, wallet_maker.get_unconfirmed_balance, MAKER_CUNT_BALANCE)
        ...
        await time_out_assert(15, wallet_taker.get_confirmed_balance, TAKER_CUNT_BALANCE)
        await time_out_assert(15, wallet_taker.get_unconfirmed_balance, TAKER_CUNT_BALANCE)
        ...
        assert len(taker_txs) == 3  # One for each: the outgoing CAT, the incoming cunt, and the outgoing cunt fee

        # cat_for_cunt
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cat_for_cunt)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        MAKER_CAT_BALANCE -= 4
        MAKER_CUNT_BALANCE += 3
        TAKER_CAT_BALANCE += 4
        TAKER_CUNT_BALANCE -= 3
        ...
        assert len(maker_txs) == 1  # The other side will show up as a regular incoming transaction
        assert len(taker_txs) == 2  # One for each: the outgoing cunt, the incoming CAT
        ...
        # cunt_for_multiple_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cunt_for_multiple_cat)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        MAKER_CUNT_BALANCE -= 7
        MAKER_CAT_BALANCE += 8
        MAKER_NEW_CAT_BALANCE += 9
        TAKER_CUNT_BALANCE += 7
        TAKER_CAT_BALANCE -= 8
        ...
        # multiple_cat_for_cunt
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(multiple_cat_for_cunt)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        # cunt_and_cat_for_cat
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cunt_and_cat_for_cat)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        MAKER_CUNT_BALANCE = 20 * 1000000000000 - 100
        MAKER_CAT_BALANCE = 100
        TAKER_CUNT_BALANCE = 20 * 1000000000000
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_CUNT_BALANCE)
        ...
        await time_out_assert(15, wallet_maker.get_confirmed_balance, MAKER_CUNT_BALANCE - FEE)
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, MAKER_CAT_BALANCE)
        await time_out_assert(15, wallet_taker.get_confirmed_balance, TAKER_CUNT_BALANCE)
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cunt_for_cat)
        await asyncio.sleep(1)
        assert error is None
        assert success is True
        assert trade_make is not None
