...
from cunt.wallet.transaction_sorting import SortKey
from cunt.wallet.util.compute_memos import compute_memos
...
class TestWalletRpc:
    ...
    async def test_wallet_rpc(self, two_wallet_nodes, trusted):
        ...
        try:
            ...
            addr = encode_puzzle_hash(await wallet_node_2.wallet_state_manager.main_wallet.get_new_puzzlehash(), "vag")
            tx_amount = 15600000
            try:
            ...
            for i in range(0, 5):
                await client.farm_block(encode_puzzle_hash(ph_2, "vag"))
            ...
            # Test RELEVANCE
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_2, "vag"))  # Create a pending tx
            ...
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_by_addr, "vag"))
            await client.farm_block(encode_puzzle_hash(ph_by_addr, "vag"))
            ...
            tx_for_address = await wallet_rpc_api.get_transactions(
                {"wallet_id": "1", "to_address": encode_puzzle_hash(ph_by_addr, "vag")}
            )
            ...
            # Create an offer of 5 cunt for one CAT
            ...
            assert summary == {"offered": {"vag": 5}, "requested": {col.hex(): 1}, "fees": 1}
            ...
            test_config["farmer"]["vag_target_address"] = encode_puzzle_hash(test_ph, "tvag")
            # set pool to second private key
            sk = await wallet_node.get_key_for_fingerprint(pks[1])
            test_ph = create_puzzlehash_for_pk(master_sk_to_wallet_sk(sk, uint32(0)).get_g1())
            test_config["pool"]["vag_target_address"] = encode_puzzle_hash(test_ph, "tvag")
