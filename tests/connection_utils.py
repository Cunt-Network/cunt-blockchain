...
async def disconnect_all_and_reconnect(server: CuntServer, reconnect_to: CuntServer) -> bool:
...
async def add_dummy_connection(server: CuntServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.cunt_ca_crt_path.read_bytes(), server.cunt_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.cunt_ca_crt_path, server.cunt_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: CuntServer, server_2: CuntServer) -> WSCuntConnection:
    """
