...
from cunt.cmds.cunt import cli
from cunt.cmds.keys import delete_all_cmd, generate_and_print_cmd, show_cmd, sign_cmd, verify_cmd
from cunt.util.config import load_config
from cunt.util.file_keyring import FileKeyring
from cunt.util.keychain import DEFAULT_USER, DEFAULT_SERVICE, Keychain, generate_mnemonic
from cunt.util.keyring_wrapper import DEFAULT_KEYS_ROOT_PATH, KeyringWrapper, LegacyKeyring
...
class TestKeysCommands:
    ...
    def empty_keyring(self):
        with TempKeyring(user="user-cunt-1.8", service="cunt-user-cunt-1.8") as keychain:
            yield keychain
    ...
    def test_generate_with_new_config(self, tmp_path, empty_keyring):
        ...
        the correct vag_target_address entries.
        """
        ...
        # Verify that the config has the correct vag_target_address entries
        address_matches = re.findall(r"vag1[^\n]+", result.output)
        assert len(address_matches) > 1
        address = address_matches[0]

        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["vag_target_address"] == address
        assert config["pool"]["vag_target_address"] == address
    ...
    def test_generate_with_existing_config(self, tmp_path, empty_keyring):
        ...
        the original vag_target_address entries.
        """
        ...
        assert existing_config["farmer"]["vag_target_address"] == address
        assert existing_config["pool"]["vag_target_address"] == address
        ...
        # Verify that the config's vag_target_address entries have not changed
        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["vag_target_address"] == existing_config["farmer"]["vag_target_address"]
        assert config["pool"]["vag_target_address"] == existing_config["pool"]["vag_target_address"]

    def test_show(self, keyring_with_one_key):
        """
        Test that the `cunt keys show` command shows the correct key.
        """
    ...
    def test_show_mnemonic(self, keyring_with_one_key):
        """
        Test that the `cunt keys show --show-mnemonic-seed` command shows the key's mnemonic seed.
        """
    ...
    def test_generate_and_print(self):
        """
        Test the `cunt keys generate_and_print` command.
        """
    ...
    def test_sign(self, keyring_with_one_key):
        """
        Test the `cunt keys sign` command.
        """
    ...
    def test_sign_non_observer(self, keyring_with_one_key):
        """
        Test the `cunt keys sign` command with a non-observer key.
        """
    ...
    def test_verify(self):
        """
        Test the `cunt keys verify` command.
        """
    ...
    def test_derive_search(self, tmp_path, keyring_with_one_key):
        """
        Test the `cunt keys derive search` command, searching a public and private key
        """
    ...
    def test_derive_search_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `cunt keys derive search` command, searching for a wallet address
        """
        ...
        result: Result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "vag1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd",
            ],
        )
        ...
        assert (
            result.output.find(
                (
                    ...
                    "vag1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd (HD path: m/12381/8444/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_failure(self, tmp_path, keyring_with_one_key):
        """
        Test the `cunt keys derive search` command with a failing search.
        """
    ...
    def test_derive_search_hd_path(self, tmp_path, empty_keyring, mnemonic_seed_file):
        """
        Test the `cunt keys derive search` command, searching under a provided HD path.
        """
    ...
    def test_derive_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `cunt keys derive wallet-address` command, generating a couple of wallet addresses.
        """
        ...
        assert (
            result.output.find(
                (
                    ...
                    "vag1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfs6wrfdq"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    ...
                    "vag1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8sumvfmt"
                )
            )
            != -1
        )
    ...
    def test_derive_child_keys(self, tmp_path, keyring_with_one_key):
        """
        Test the `cunt keys derive child-keys` command, generating a couple of derived keys.
        """
    ...
    def test_migration_not_needed(self, tmp_path, setup_keyringwrapper, monkeypatch):
        """
        Test the `cunt keys migrate` command when no migration is necessary
        """
    ...
    def test_migration_full(self, tmp_path, setup_legacy_keyringwrapper):
        """
        Test the `cunt keys migrate` command when a full migration is needed
        """
        ...
        current_version_str = pkg_resources.get_distribution("cunt-blockchain").version
    ...
    def test_migration_incremental(self, tmp_path, keyring_with_one_key, monkeypatch):
        ...
        from cunt.util import keyring_wrapper
    ...
    def test_migration_checked(self, tmp_path, monkeypatch):
        ...
        assert pkg_resources.get_distribution("cunt-blockchain").version == "1.2.11.dev123"
