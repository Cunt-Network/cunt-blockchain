...
- Generators have been re-worked in CLVM. We added a chialisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during CuntLisp native deserialization.
...
- Cunt wheels not on Pypi and some dependecies not found there also are now on pypi.cuntcoin.net.
...
- RC3 is a new chain to support the last major chialisp changes. TVAG from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- 'cunt wallet send' now takes in TVAG or VAG as units instead of bytes.
